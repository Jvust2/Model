(function () {
  ...
  function runtimeUrl(path) { return runtimeBase.replace(/\/$/, "") + path; }

  function clearRuntimePoll() {
    if (runtimePollTimer) { clearTimeout(runtimePollTimer); runtimePollTimer = null; }
  }

  function scheduleRuntimeRefresh() {
    clearRuntimePoll();
    if (runtimeState && runtimeState.running && runtimeState.phase === "loading") {
      runtimePollTimer = setTimeout(() => refreshRuntime().catch(() => {}), 1000);
    }
  }

  function runtimeLabel(state) {
    if (!state) return "未连接本地 Runtime";
    if (state.running && state.ready) return "已就绪 · " + (state.model || "GGUF");
    if (state.running && state.phase === "loading") return "加载中 · " + (state.model || "GGUF");
  ...
  function runtimeLogText(state) {
    ...
    if (state && state.logs && state.logs.length) lines.push(...state.logs.slice(-12));
  ...
  async function sendChat() {
    ...
    try {
      ...
      if (!response.ok) {
        const upstream = data.error && data.error.message ? data.error.message : data.error;
        throw new Error(upstream || "生成失败：" + response.status);
      }
      const contentOut = data.choices && data.choices[0] && data.choices[0].message
        ? data.choices[0].message.content
        : "";
      if (!contentOut) throw new Error("模型返回了空响应。");
      chatHistory.push({ role: "assistant", content: String(contentOut) });
      renderChat();
      setStatus("本机模型回复完成");
    } catch (error) {
      showError(error);
  ...
  async function inspectModel(model, button) {
    ...
    try {
      ...
      setStatus("GGUF v" + info.version + " · " + info.tensor_count + " tensors · " + info.metadata_kv_count + " metadata");
    } catch (error) {
      showError(error);
      setStatus("GGUF 检查失败");
    } finally {
  ...
  function renderModels() {
    ...
    for (const model of models) {
      ...
      if (runtimeState && runtimeState.running && runtimeState.model_relative_path === model.relativePath) card.classList.add("active");
  ...
  async function scanDrive() {
    ...
    try {
      ...
      const result = await window.DriveModelClient.scanModelTree(accessToken, rootId, progress => {
        setStatus("扫描中 · " + progress.scannedFolders + " 文件夹 · " + progress.modelFiles + " 模型");
      });
      ...
      renderModels();
      setStatus("扫描完成 · " + result.scannedFolders + " 文件夹 · " + result.modelFiles + " 模型");
    } catch (error) {
      showError(error);
      setStatus("扫描失败");
  ...
  async function startModel(model) {
    ...
    try {
      const response = await fetch(runtimeUrl("/v1/models/start"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ drive_file_id: model.id, name: model.name, relative_path: model.relativePath, size: Number(model.size || 0), format: model.runnableFormat })
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Runtime 启动失败：" + response.status);
      setStatus(data.ready ? "本机模型已就绪 · PID " + data.pid : "本机模型正在加载 · PID " + data.pid);
  ...
  async function stopModel() {
    ...
    clearRuntimePoll();
    try {
      const response = await fetch(runtimeUrl("/v1/models/stop"), { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
      const data = await response.json().catch(() => ({}));
  ...
  async function init() {
    ...
    runtimeBase = localStorage.getItem("model_runtime_base") || window.MODEL_CONFIG.runtimeBase || "http://127.0.0.1:8765";
    $("runtimeUrl").value = runtimeBase;
  ...
  $("loginBtn").addEventListener("click", () => { try { window.DriveModelClient.authorizeDrive(); } catch (error) { showError(error); } });
  $("scanBtn").addEventListener("click", scanDrive);
  $("runtimeCheckBtn").addEventListener("click", async () => { saveRuntimeBase(); const ok = await refreshRuntime(); setStatus(ok ? "本机 Runtime 已连接" : "无法连接本机 Runtime"); });
